...
for d in d_matrix:
    for N in N_matrix:
        n = d
        if d == 8:
            true = np.array([1.5, 1.5, 1.0, 1.5, 1.5, 1.5, 1.5, 1.5, 0.3, 0.1, 0.1])
            RF_index = np.array(['Y', 'Y', 'N', 'N', 'N', 'N', 'N', 'N'])
        else:
            true = np.array([1.5, 1.5, 1.0, 1.5, 1.5, 0.3, 0.1, 0.1])
        ...
        np.random.seed(2019)
        gamma1 = np.random.laplace(0, true[-2], size = g)

        np.random.seed(2020)
        gamma2 = np.random.laplace(0, true[-1], size = g)
        #####
        ...
        for t in range(M):
            ...
            #np.corrcoef(X1.T)
            Z = X[:, 0:k]
            #E = (X@beta_v[:, np.newaxis]).ravel() + Z*np.repeat(gamma_h, int(N/g))
            ...
            lb = [tol]*d + [-100] + [tol]*k + [-100]*N 
            ub = [100]*d + [100] + [100]*k + [100]*N
            ...
            for j in range(len(iniseed)):
                ...
                tem10 = uniform.rvs(loc=true[-1], scale=0.02, size=k, random_state=int(iniseed[j]))
            ...
            if res is None:
                ...
                #print(b_v)
                ##########
                step2_obj = step2_RF(res[0:d], np.array([sigma_hat]), b_v, g, X, Y, Z_long)
                lb_2 = np.zeros(2*k*g)
                ...
                x1_rf = np.zeros(g)
                v1 = np.abs(x1_rf)+0.001
                x0_final = np.concatenate((v0, x0_rf, v1, x1_rf))
                #x0_rf = np.zeros(g)
                #v0 = np.abs(x0_rf)+0.001
                #x0_final = np.concatenate((v0, x0_rf))
                ##############
                meth2 = minimize(fun = step2_obj.RF_step2_laplace_k2, x0 = x0_final, method='trust-constr', constraints = [step2_constraint], options = {'xtol': 1e-10, 'gtol':1e-10, 'maxiter': 2000})
                rfs1 = np.concatenate((meth2.x[g:2*g], meth2.x[3*g:4*g]))    
                ##########
                meth3 = minimize(fun = RF_step2_laplace_simple, x0 = np.zeros(k*g), args = (res[0:d], np.array([sigma_hat]), g, X, Y, Z_long), method='trust-constr', options = {'xtol': 1e-10, 'gtol':1e-10, 'maxiter': 2000})
                rfs2 = meth3.x
                gamma_vr = gamma_v.ravel() 
                rmse1 = np.sqrt(np.sum((rfs1 - gamma_vr)**2)/(g*k))
                rmse2 = np.sqrt(np.sum((rfs2 - gamma_vr)**2)/(g*k)) 
        ...
        name = '_'.join(['Laplace_slope', 'd', str(d), 'N', str(N)])
